...
class NSFW(commands.Cog):
    ...
    @commands.command(usage='[tag]', brief='Get an image from Gelbooru with the specified tag(s).', description='Get an image from Gelbooru with the specified tag(s). Multiple tags can be appended with + e.g. tag1+tag2. Leave blank for random image.')
    @commands.is_nsfw()
    ...
    @commands.command(aliases=['hateit', 'undo'], brief='Delete the image you recently requested.', description='Delete the image you recently requested.')
    @commands.is_nsfw()
