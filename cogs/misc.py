...
class Misc(commands.Cog):
    ...
    @commands.command(name='userinfo', usage='[user]', brief='Shows information about a user.', description='Shows information about a user.')
    @commands.guild_only()
    ...
    async def user_info_error(self, ctx, error):
        if isinstance(error, commands.BadArgument):
            return await ctx.send(f"{ctx.author.mention} you did not enter a valid user.")

    # Gets a user's avatar
    @commands.command(usage='[user]', brief='Gets a user\'s avatar.', description='Gets a user\'s avatar.')
    @commands.guild_only()
